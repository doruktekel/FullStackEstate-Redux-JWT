...
const Home = () => {
  ...
  const fetchData = async (type, setter) => {
    try {
      const res = await axios.get(`/api/list/get?${type}&limit=4`);
      setter(res.data);
    } catch (error) {
      console.log(error);
    }
  ...
  useEffect(() => {
    fetchData("type=sale", setSaleListings);
    fetchData("type=rent", setRentListings);
    fetchData("offer=true", setOfferListings);
  }, []);
